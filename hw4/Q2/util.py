...
def entropy(class_y):
    ...
    entropy = 0
    """ use dictionary store the count of every value in class_y"""
    d = {}
...
def partition_classes(X, y, split_attribute, split_val):
    ...
    """Numeric Split Attribute"""
    if type(split_val) is int or type(split_val) is float:
        ...
        for i in range(len(X)):
            if X[i][split_attribute] is split_val:
                X_left.append(X[i])
                y_left.append(y[i])
            else:
                X_right.append(X[i])
...
def information_gain(previous_y, current_y):
    ...
    for i in range(len(current_y)):
        ...
        info_gain -= p * entropy(current_y[i])
    return info_gain
